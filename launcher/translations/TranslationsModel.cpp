...
void TranslationsModel::reloadLocalFiles()
{
    ...
    QList<QDir> translationDirs = { d->m_dir };
    if (d->m_install_dir.exists()) {
    ...
    QMap<QString, TranslationCandidate> candidates;

    for (int directoryIndex = 0; directoryIndex < translationDirs.size(); ++directoryIndex) {
        const auto& directory = translationDirs.at(directoryIndex);
        const auto entries = directory.entryInfoList({ "mmc_*.qm", "*.po", "!*.po" }, QDir::Files | QDir::NoDotAndDotDot);
        for (const auto& entry : entries) {
            const auto completeSuffix = entry.completeSuffix();
            QString fileName = entry.fileName();
            const bool isPriority = fileName.startsWith('!');
            ...
            if (completeSuffix == "qm") {
                if (fileName.startsWith("mmc_")) {
                    langCode = QFileInfo(fileName.mid(4)).baseName();
                } else {
                    continue;
                }
                fileType = FileType::Qm;
            } else if (completeSuffix == "po") {
                langCode = QFileInfo(fileName).baseName();
                fileType = FileType::Po;
            } else {
                continue;
            }

            const int priority = (isPriority ? 100 : 0) + (directoryIndex == 0 ? 20 : 10) +
                                 (fileType == FileType::Po ? 2 : 1);
            if (!candidates.contains(langCode) || candidates.value(langCode).priority < priority) {
                candidates.insert(langCode, { fileType, entry.absoluteFilePath(), priority });
            }
        }
    }

    for (auto candidate = candidates.cbegin(); candidate != candidates.cend(); ++candidate) {
        const auto& langCode = candidate.key();
        auto langIter = languages.find(langCode);
        if (langIter == languages.end()) {
            langIter = languages.insert(langCode, Language(langCode));
        }
        langIter->localFileType = candidate->type;
        langIter->localFilePath = candidate->path;
    }
...
bool TranslationsModel::selectLanguage(QString key) const
{
    ...
    if (langPtr->localFileType == FileType::Po) {
        ...
        d->m_appTranslator = std::make_unique<POTranslator>(langPtr->localFilePath);
        ...
        if (d->m_appTranslator->load(langPtr->localFilePath)) {
            qDebug() << "Loading Application Language File for" << langCode.toLocal8Bit().constData() << "...";
            if (!QCoreApplication::installTranslator(d->m_appTranslator.get())) {
                qCritical() << "Installing Application Language File failed.";
                d->m_appTranslator.reset();
